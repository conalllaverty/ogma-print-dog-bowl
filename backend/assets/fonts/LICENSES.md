# Bundled font licenses

Production font files are redistributed under their upstream open licenses.
The complete terms are included as `OFL-1.1.txt` and `Apache-2.0.txt`.

- **Overpass** — SIL Open Font License 1.1  
  https://github.com/RedHatOfficial/Overpass
- **Source Sans 3** — SIL Open Font License 1.1  
  https://github.com/adobe-fonts/source-sans
- **Lora** — SIL Open Font License 1.1  
  https://github.com/cyrealtype/Lora-Cyrillic
- **Roboto Slab** — Apache License 2.0  
  https://github.com/googlefonts/robotoslab
- **Fredoka** — SIL Open Font License 1.1  
  https://github.com/hafontia-zz/Fredoka-One
- **Baloo 2** — SIL Open Font License 1.1  
  https://github.com/EkType/Baloo2
- **Barlow Condensed** — SIL Open Font License 1.1  
  https://github.com/jpt/barlow

The font binaries remain subject to their upstream copyright notices and
license terms. Font names and files must not be relicensed as project code.
